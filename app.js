...
database.ref(refPlayers).on("child_removed", function(snapshot) {
    ...
    if (pID !== playerID) {
        if (
            /make your selection|Read and wait/.test(
        ...
        opponentEntry.attr(
            "placeholder",
            "Awaiting for another player to join"
        );
    }
...
function playerEntry(playerNum) {
    const playerName = $(`#player${playerNum}-entry`)
        .val()
        .trim();
...
function player1Entry() {
    const playerName = $("#player1-entry")
        .val()
        .trim();
...
function player2Entry() {
    const playerName = $("#player2-entry")
        .val()
        .trim();
...
database.ref(refGame).on("value", snapshot => {
    ...
    if (selections) {
        ...
        if (selectionLength === 2) {
            ...
            if (numOfPlayers() == 2) {
                const msg =
                    "Please make your selection. Your opponent is waiting!";
