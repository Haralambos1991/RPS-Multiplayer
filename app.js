...
// CHAT LISTENERS
// Chat send button listener, grabs input and pushes to firebase. (Firebase's push automatically ctrates a new key)
...
//User has chosen, so removes choices and display what they chose
...
chatData.orderByChild("time").on("child_added", function(snapshot) {
   ...
   // Keeps div scrolles to bottom on each update//
   $("#chat-messages").scrollTop($("#chat-messages")[0].scrollHeight);
});

// Tracks changes in key which contain player objects
playersRef.on("value", function(snapshot) {
  // lenght of the 'players' array
  ...
  // if there is player two, fill in name and win/loss data
  if (playerTwoExists) {
     $("player2-name").text(playerTwoData.name);
     $("#player2-wins").text("Wins: " + playerTwoData.losses);
  } else {
...
if (playerNum) {
   ...
   if (currentTurn === 1) {
     ...
     if (currentTurn === playerNum) {
      ...
      // If it isn't the current player's turn, tells them they're waiting for player one
      $("# current-turn").text("Waiting for " + playerTowData.name +  " to choose.");
     }
...
function getInGame() {
    ...
    // if there is no player one, then the user becomes player one.
    if (currentPlayers < 2) {
       if (playerOneExists) {
           playerNum = 2;
       } else {
       ...
       // On disconnect remove the user's player object
...
function gameLogic(playerchoice, player2choice) {
...
if (player1choice === "Rock" && player2choice === "Rock") {
    ...
    playerOneWon();
} else if (pla1choice ===  "Paper"  && player2choice === "Rock") {
    playerOneWon();
